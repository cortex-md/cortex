			...
			if (vaultPath && config.enabled && config.remoteVaultId) {
				...
				if (remote) {
					return {
						...
						lastEditedBy:
							remote.lastEditedBy ?? (remote.lastDeviceId ? `device:${remote.lastDeviceId}` : null),
					}
				}
			}
			return {
				...
				createdBy: `device:${await platform.device.getDeviceId()}`,
				lastEditedAt: new Date(fileMetadata.modifiedAt).toISOString(),
				lastEditedBy: `device:${await platform.device.getDeviceId()}`,
			}
		},
		getDeviceId: () => platform.device.getDeviceId(),
	})
}
