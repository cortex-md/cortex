...
import type { SidebarViewItem } from "../../../features/file-explorer/SidebarViewCarousel"
import {
	getSidebarViewDirection,
	SidebarViewStage,
} from "../../../features/file-explorer/SidebarViewStage"
...
const renderView = (id: string) => <div data-testid={`view-${id}`}>{id}</div>
...
describe("SidebarViewStage", () => {
	it("animates both panels in the selected direction", () => {
		const { rerender } = render(
			<SidebarViewStage activeId="files" items={items} renderView={renderView} />,
		)

		rerender(<SidebarViewStage activeId="search" items={items} renderView={renderView} />)
	...
	it("keeps only the latest two panels during rapid navigation", () => {
		...
		rerender(<SidebarViewStage activeId="search" items={items} renderView={renderView} />)
		rerender(<SidebarViewStage activeId="tags" items={items} renderView={renderView} />)
