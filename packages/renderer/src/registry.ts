...
export function validateMarkdownProcessorRegistration(
	...
	if (
		...
		registration.surfaces.some((surface) => surface !== "reading-view" && surface !== "export")
	) {
		throw new Error(
			'Markdown processors support only "reading-view" and "export"; use registerSemantic or editor.registerExtension for Live Preview',
		)
	}
...
export function getMarkdownProcessorEntries(
	...
	return sortEntries(processorEntries)
		.filter((entry) => !surface || entry.value.surfaces.includes(surface))
		.filter((entry) => !phase || entry.value.phase === phase)
		.map((entry) => ({ namespace: entry.namespace, registration: entry.value }))
}
...
function getInlineTextTransforms(source: string): MarkdownTextTransform[] {
	...
	for (const entry of sortEntries(inlineEntries)) {
		const regex = compileMarkdownInlineRegistration(entry.value)
		let matches = 0
		for (let match = regex.exec(source); match !== null; match = regex.exec(source)) {
			...
			try {
				const replacement = entry.value.replacement
