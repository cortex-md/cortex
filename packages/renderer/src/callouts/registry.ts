...
function buildEffectiveCalloutTypes(): CalloutTypeDefinition[] {
	...
	for (const entry of registrations) {
		...
		const aliasOwner = definitions.find(
			(definition) =>
				definition.type === registrationType || definition.aliases.includes(registrationType),
		)
		...
		const aliases = Array.from(
			new Set([
				...(existing?.aliases ?? []),
				...(entry.registration.aliases ?? [])
					.map(normalizeCalloutName)
					.filter((alias) => alias && alias !== canonicalType),
			]),
		)
		...
		if (existing) {
			const index = definitions.indexOf(existing)
