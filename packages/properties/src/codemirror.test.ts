...
describe("createFrontmatterExtension", () => {
	it("keeps metadata outside the CodeMirror document", () => {
		const onChange = vi.fn()
		const view = createView(onChange)
		expect(view.state.doc.toString()).toBe(body)
		expect(getFrontmatterEditorState(view.state)).toEqual({
	...
	it("updates metadata without changing the document or undo history", () => {
		const onChange = vi.fn()
		const view = createView(onChange)
		updateFrontmatterEditorState(view, { title: "Remote" })
		expect(view.state.doc.toString()).toBe(body)
		expect(getFrontmatterEditorState(view.state)).toEqual({
			meta: { title: "Remote" },
			error: null,
	...
	it("publishes malformed frontmatter state without blocking body edits", () => {
		const onError = vi.fn()
		const view = createView(undefined, onError)
		updateFrontmatterEditorState(view, {}, "Malformed YAML")
